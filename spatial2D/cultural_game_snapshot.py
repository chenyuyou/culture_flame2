...
SINGLE_RUN_PARAMETERS = {
    ...
    "snapshot_filename": "end_snapshot.json" # Filename for the snapshot
}
# ==============================================================================
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a single FLAMEGPU2 Cultural Game Simulation and export final snapshot.")
    parser.add_argument("--output_dir", type=str, default=SINGLE_RUN_PARAMETERS["output_directory"],
                        help=f"Directory to save the final snapshot file. Default: {SINGLE_RUN_PARAMETERS['output_directory']}")
    parser.add_argument("--snapshot_filename", type=str, default=SINGLE_RUN_PARAMETERS["snapshot_filename"],
                        help=f"Filename for the final snapshot JSON. Default: {SINGLE_RUN_PARAMETERS['snapshot_filename']}")
    ...
    current_params["snapshot_filename"] = args.snapshot_filename
    ...
    # Pass the RunPlan to simulate()
    cuda_sim.simulate(run_plan)
    end_time = time.time()
    ...
    snapshot_filename = current_params["snapshot_filename"]
    os.makedirs(output_directory, exist_ok=True) # Ensure output directory exists
    snapshot_path = os.path.join(output_directory, snapshot_filename)

    print(f"Exporting final simulation state to: {snapshot_path}")
    try:
