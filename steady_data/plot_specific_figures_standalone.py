...
def plot_time_series(aggregated_ts_data, param_combo_id, metric_name, metric_label, output_dir="figures"):
    """
    Plots the time series of a specific metric for a given parameter combination.
    aggregated_ts_data is the dictionary returned by process_logs.py
    param_combo_id is the key in the dictionary (the directory name)
    metric_name is the base name of the metric (e.g., 'coop_rate')
    """
    # print(f"\n--- Plotting Time Series for {metric_name} for combination: {param_combo_id} ---") # Moved print outside loop

    if not aggregated_ts_data or param_combo_id not in aggregated_ts_data:
        print(f"No time series data found for combination: {param_combo_id}")
        return

    ts_df = aggregated_ts_data[param_combo_id]

    mean_col = f'{metric_name}_mean'
    sem_col = f'{metric_name}_sem'

    required_cols = ['step', mean_col, sem_col]
    if not all(col in ts_df.columns for col in required_cols):
        print(f"Missing required columns for time series plot for {param_combo_id}. Need: {required_cols}")
        print(f"Available columns: {ts_df.columns.tolist()}")
        return

    plt.figure(figsize=(8, 5)) # Adjust figure size for time series

    # Plot the mean time series
    plt.plot(ts_df['step'], ts_df[mean_col], label=f'Mean {metric_label}')

    # Plot the shaded area for SEM
    # Ensure SEM values are not NaN or Inf before plotting
    sem_values = ts_df[sem_col].replace([np.inf, -np.inf], np.nan).fillna(0)
    if np.any(sem_values > 0):
        plt.fill_between(ts_df['step'], ts_df[mean_col] - sem_values, ts_df[mean_col] + sem_values, color='gray', alpha=0.3, label='SEM')

    plt.xlabel('Time Step')
    plt.ylabel(metric_label)
    plt.title(f'Time Series of {metric_label} for {param_combo_id}')
    plt.legend(loc='best')
    plt.grid(True, linestyle='--', alpha=0.6)
    ...
    # Save the figure
    os.makedirs(output_dir, exist_ok=True)
    # Sanitize param_combo_id for filename
    safe_param_combo_id = param_combo_id.replace('_', '-').replace('.', 'p')
    fig_path_png = os.path.join(output_dir, f"timeseries_{metric_name.lower()}_{safe_param_combo_id}.png")
    fig_path_pdf = os.path.join(output_dir, f"timeseries_{metric_name.lower()}_{safe_param_combo_id}.pdf")
    plt.savefig(fig_path_png, dpi=300, bbox_inches='tight')
    plt.savefig(fig_path_pdf, dpi=300, bbox_inches='tight')
    # print(f"Saved time series figure to {fig_path_png} and {fig_path_pdf}") # Moved print outside loop
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_file_steady_state", type=str,
    ...
    parser.add_argument("--data_file_timeseries", type=str,
                        help="Path to the aggregated time series data pickle file generated by process_logs.py")
    ...
    parser.add_argument("--plot", nargs='+', default=['all'],
                        choices=['all', 'fig1a', 'fig1b_type', 'fig1b_strategy', 'fig2a', 'fig2b_type', 'fig2b_strategy', 'fig7a', 'fig7b', 'phasediagram_seg_type', 'phasediagram_boundary_frac', 'timeseries', 'all_timeseries'],
                        help="Specify which plots to generate. 'all' plots all available figures. 'all_timeseries' plots time series for all parameter combinations. Choose from: fig1a, fig1b_type, fig2a, fig2b_type, fig2b_strategy, fig7a, fig7b, phasediagram_seg_type, phasediagram_boundary_frac, timeseries, all_timeseries")
    ...
    # Add arguments for time series plotting (for a single combination)
    parser.add_argument("--ts_param_combo_id", type=str,
                        help="Parameter combination ID (directory name) for time series plotting (when using --plot timeseries).")
    parser.add_argument("--ts_metric", type=str,
                        help="Metric to plot time series for (e.g., 'coop_rate', 'avg_C') (when using --plot timeseries or all_timeseries).")
    ...
    # Load steady state data if requested
    steady_state_plots_requested = any(p in args.plot for p in ['all', 'fig1a', 'fig1b_type', 'fig1b_strategy', 'fig2a', 'fig2b_type', 'fig2b_strategy', 'fig7a', 'fig7b', 'phasediagram_seg_type', 'phasediagram_boundary_frac'])
    if steady_state_plots_requested:
        if not args.data_file_steady_state:
            print("Error: --data_file_steady_state is required for steady state or 'all' plots.")
            sys.exit(1)
        try:
            ...
            print(f"Steady State DataFrame shape: {processed_df.shape}")
            print(f"Available columns for steady state plotting: {processed_df.columns.tolist()}")
    ...
    # Load time series data if requested
    time_series_plots_requested = any(p in args.plot for p in ['all', 'timeseries', 'all_timeseries'])
    if time_series_plots_requested:
         if not args.data_file_timeseries:
             print("Error: --data_file_timeseries is required for time series or 'all' plots.")
             sys.exit(1)
         try:
             with open(args.data_file_timeseries, 'rb') as f:
                 aggregated_time_series_data = pickle.load(f)
             print(f"Successfully loaded time series data from {args.data_file_timeseries}")
             print(f"Aggregated Time Series data available for {len(aggregated_time_series_data)} parameter combinations.")
             # Print available parameter combination IDs for time series plotting
             print(f"Available parameter combination IDs for time series plotting: {list(aggregated_time_series_data.keys())}")

         except FileNotFoundError:
             print(f"Error: Time series data file not found at {args.data_file_timeseries}")
             sys.exit(1) # Exit if time series data is required but not found
         except Exception as e:
             print(f"Error loading time series data from {args.data_file_timeseries}: {e}")
             sys.exit(1)
    ...
    if 'all' in plots_to_generate:
        # Define all possible plot types when 'all' is selected
        plots_to_generate = ['fig1a', 'fig1b_type', 'fig2a', 'fig2b_type', 'fig2b_strategy', 'fig7a', 'fig7b', 'phasediagram_seg_type', 'phasediagram_boundary_frac', 'all_timeseries'] # Changed 'timeseries' to 'all_timeseries' for 'all'

    # Define a mapping from metric name to its label for plotting
    metric_labels = {
        'coop_rate': 'Average Cooperation Rate',
        'avg_C': 'Average Culture',
        'segregation_index_strategy': 'Segregation Index (Strategy)',
        'segregation_index_type': 'Segregation Index (Type)',
        'boundary_fraction': 'Boundary Fraction',
        'boundary_coop_rate': 'Boundary Cooperation Rate',
        'bulk_coop_rate': 'Bulk Cooperation Rate',
    }
    ...
    # --- Add logic for plotting Time Series ---
    if 'timeseries' in plots_to_generate:
        if not args.ts_param_combo_id or not args.ts_metric:
            print("\nWarning: --ts_param_combo_id and --ts_metric are required for plotting a single time series. Skipping single time series plot.")
        else:
            metric_label = metric_labels.get(args.ts_metric, args.ts_metric)
            plot_time_series(aggregated_time_series_data, args.ts_param_combo_id, args.ts_metric, metric_label, args.output_dir)

    if 'all_timeseries' in plots_to_generate:
        if not args.ts_metric:
            print("\nWarning: --ts_metric is required for plotting all time series. Skipping all time series plots.")
        elif not aggregated_time_series_data:
             print("\nWarning: No aggregated time series data loaded. Skipping all time series plots.")
        else:
            metric_label = metric_labels.get(args.ts_metric, args.ts_metric)
            print(f"\n--- Plotting Time Series for metric: {args.ts_metric} for all parameter combinations ---")
            for param_combo_id in tqdm(aggregated_time_series_data.keys(), desc=f"Plotting Time Series for {args.ts_metric}"):
                 plot_time_series(aggregated_time_series_data, param_combo_id, args.ts_metric, metric_label, args.output_dir)
            print(f"\nFinished plotting time series for metric: {args.ts_metric} for all parameter combinations.")
