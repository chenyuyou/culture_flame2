...
FLAMEGPU_AGENT_FUNCTION(calculate_local_metrics_func, flamegpu::MessageArray2D, flamegpu::MessageNone) {
    ...
    // Define the thresholds for the four cultural types
    const float C_threshold_1 = 0.25f;
    const float C_threshold_2 = 0.50f;
    const float C_threshold_3 = 0.75f;

    // Determine own type (1, 2, 3, or 4) based on the new thresholds
    unsigned int self_type;
    if (self_C >= 0.0f && self_C < C_threshold_1) self_type = 1u;
    else if (self_C >= C_threshold_1 && self_C < C_threshold_2) self_type = 2u;
    else if (self_C >= C_threshold_2 && self_C < C_threshold_3) self_type = 3u;
    else self_type = 4u; // 0.75 <= self_C <= 1.0
    ...
    // Use 1, 2, 3, 4 for the four types
    unsigned int is_type_1 = (self_type == 1u) ? 1u : 0u;
    unsigned int is_type_2 = (self_type == 2u) ? 1u : 0u;
    unsigned int is_type_3 = (self_type == 3u) ? 1u : 0u;
    unsigned int is_type_4 = (self_type == 4u) ? 1u : 0u;
    ...
    for (const auto& neighbor_msg : FLAMEGPU->message_in.wrap(self_ix, self_iy, 1)) {
        ...
        // Determine neighbor's type (1, 2, 3, or 4)
        unsigned int neighbor_type;
        if (neighbor_C >= 0.0f && neighbor_C < C_threshold_1) neighbor_type = 1u;
        else if (neighbor_C >= C_threshold_1 && neighbor_C < C_threshold_2) neighbor_type = 2u;
        else if (neighbor_C >= C_threshold_2 && neighbor_C < C_threshold_3) neighbor_type = 3u;
        else neighbor_type = 4u; // 0.75 <= neighbor_C <= 1.0
    ...
    FLAMEGPU->setVariable<unsigned int>("cooperator_type_1", is_type_1 * is_cooperator);
    FLAMEGPU->setVariable<unsigned int>("cooperator_type_2", is_type_2 * is_cooperator);
    FLAMEGPU->setVariable<unsigned int>("cooperator_type_3", is_type_3 * is_cooperator);
    FLAMEGPU->setVariable<unsigned int>("cooperator_type_4", is_type_4 * is_cooperator);

    // Sum of agents per type (for normalization)
    FLAMEGPU->setVariable<unsigned int>("count_type_1", is_type_1);
    FLAMEGPU->setVariable<unsigned int>("count_type_2", is_type_2);
    FLAMEGPU->setVariable<unsigned int>("count_type_3", is_type_3);
    FLAMEGPU->setVariable<unsigned int>("count_type_4", is_type_4);

    // Sum of same_type_neighbors per type
    FLAMEGPU->setVariable<unsigned int>("same_type_neighbors_type_1", is_type_1 * same_type_count);
    FLAMEGPU->setVariable<unsigned int>("same_type_neighbors_type_2", is_type_2 * same_type_count);
    FLAMEGPU->setVariable<unsigned int>("same_type_neighbors_type_3", is_type_3 * same_type_count);
    FLAMEGPU->setVariable<unsigned int>("same_type_neighbors_type_4", is_type_4 * same_type_count);

    // Sum of total_neighbors_count per type
    FLAMEGPU->setVariable<unsigned int>("total_neighbors_count_type_1", is_type_1 * total_neighbors);
    FLAMEGPU->setVariable<unsigned int>("total_neighbors_count_type_2", is_type_2 * total_neighbors);
    FLAMEGPU->setVariable<unsigned int>("total_neighbors_count_type_3", is_type_3 * total_neighbors);
    FLAMEGPU->setVariable<unsigned int>("total_neighbors_count_type_4", is_type_4 * total_neighbors);
