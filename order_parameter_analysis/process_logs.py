...
def process_flamegpu_logs(output_directory, steady_state_window=500):
    ...
    # **NEW** Lists for group-specific steady state means
    coop_rate_A_steady_state_means = []
    coop_rate_B_steady_state_means = []
    segregation_index_type_A_steady_state_means = []
    segregation_index_type_B_steady_state_means = []
    ...
    for run_id, log_file in enumerate(tqdm(log_files, desc="Processing logs")):
        ...
        try:
            ...
            # **NEW** Group-Specific Metrics
            # Cooperation Rate Type A
            if 'coop_rate_A' in steady_state_df.columns:
                mean_coop_A = steady_state_df['coop_rate_A'].mean()
                sem_coop_A = steady_state_df['coop_rate_A'].sem()
                run_result['avg_CooperationRate_A'] = mean_coop_A
                run_result['sem_CooperationRate_A'] = sem_coop_A
                coop_rate_A_steady_state_means.append(mean_coop_A) # Collect for susceptibility
            else:
                 run_result['avg_CooperationRate_A'] = np.nan
                 run_result['sem_CooperationRate_A'] = np.nan

            # Cooperation Rate Type B
            if 'coop_rate_B' in steady_state_df.columns:
                mean_coop_B = steady_state_df['coop_rate_B'].mean()
                sem_coop_B = steady_state_df['coop_rate_B'].sem()
                run_result['avg_CooperationRate_B'] = mean_coop_B
                run_result['sem_CooperationRate_B'] = sem_coop_B
                coop_rate_B_steady_state_means.append(mean_coop_B) # Collect for susceptibility
            else:
                 run_result['avg_CooperationRate_B'] = np.nan
                 run_result['sem_CooperationRate_B'] = np.nan

            # Segregation Index Type A
            if 'segregation_index_type_A' in steady_state_df.columns:
                mean_seg_type_A = steady_state_df['segregation_index_type_A'].mean()
                sem_seg_type_A = steady_state_df['segregation_index_type_A'].sem()
                run_result['avg_SegregationIndex_A'] = mean_seg_type_A
                run_result['sem_SegregationIndex_A'] = sem_seg_type_A
                segregation_index_type_A_steady_state_means.append(mean_seg_type_A) # Collect for susceptibility
            else:
                 run_result['avg_SegregationIndex_A'] = np.nan
                 run_result['sem_SegregationIndex_A'] = np.nan

            # Segregation Index Type B
            if 'segregation_index_type_B' in steady_state_df.columns:
                mean_seg_type_B = steady_state_df['segregation_index_type_B'].mean()
                sem_seg_type_B = steady_state_df['segregation_index_type_B'].sem()
                run_result['avg_SegregationIndex_B'] = mean_seg_type_B
                run_result['sem_SegregationIndex_B'] = sem_seg_type_B
                segregation_index_type_B_steady_state_means.append(mean_seg_type_B) # Collect for susceptibility
            else:
                 run_result['avg_SegregationIndex_B'] = np.nan
                 run_result['sem_SegregationIndex_B'] = np.nan
            ...
            run_result['chi_CooperationRate_A'] = np.nan
            run_result['chi_CooperationRate_B'] = np.nan
            run_result['chi_SegregationIndex_A'] = np.nan
            run_result['chi_SegregationIndex_B'] = np.nan
    ...
    if not processed_df.empty and pd.notna(N) and N > 0:
        ...
        # **NEW** Group-Specific Susceptibility
        # Cooperation Rate Type A Susceptibility
        if coop_rate_A_steady_state_means and len(coop_rate_A_steady_state_means) > 1:
            variance_coop_A = np.var(coop_rate_A_steady_state_means, ddof=1)
            processed_df['chi_CooperationRate_A'] = N * variance_coop_A

        # Cooperation Rate Type B Susceptibility
        if coop_rate_B_steady_state_means and len(coop_rate_B_steady_state_means) > 1:
            variance_coop_B = np.var(coop_rate_B_steady_state_means, ddof=1)
            processed_df['chi_CooperationRate_B'] = N * variance_coop_B

        # Segregation Index Type A Susceptibility
        if segregation_index_type_A_steady_state_means and len(segregation_index_type_A_steady_state_means) > 1:
            variance_seg_A = np.var(segregation_index_type_A_steady_state_means, ddof=1)
            processed_df['chi_SegregationIndex_A'] = N * variance_seg_A

        # Segregation Index Type B Susceptibility
        if segregation_index_type_B_steady_state_means and len(segregation_index_type_B_steady_state_means) > 1:
            variance_seg_B = np.var(segregation_index_type_B_steady_state_means, ddof=1)
            processed_df['chi_SegregationIndex_B'] = N * variance_seg_B
